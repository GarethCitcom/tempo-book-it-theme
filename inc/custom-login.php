...
function tempo_studio_manager_login_path() {
	return (string) apply_filters( 'tempo_studio_manager_login_path', '/sign-in/' );
}
 ...
 */
function tempo_studio_manager_login_url( $redirect_to = '', $action = 'login' ) {
	$url = home_url( tempo_studio_manager_login_path() );
	if ( 'login' !== $action ) {
...
function tempo_studio_manager_is_login_request() {
	$request_path = isset( $_SERVER['REQUEST_URI'] )
		...
		: '';
	$login_path = (string) wp_parse_url( tempo_studio_manager_login_url(), PHP_URL_PATH );
...
function tempo_studio_manager_filter_login_url( $login_url, $redirect, $force_reauth ) {
	$url = tempo_studio_manager_login_url( $redirect );
	if ( $force_reauth ) {
		$url = add_query_arg( 'reauth', '1', $url );
	}
	return $url;
}
add_filter( 'login_url', 'tempo_studio_manager_filter_login_url', 10, 3 );

/** Keep WordPress-generated password recovery links inside the theme. */
function tempo_studio_manager_filter_lostpassword_url( $lostpassword_url, $redirect ) {
	return tempo_studio_manager_login_url( $redirect, 'lostpassword' );
}
add_filter( 'lostpassword_url', 'tempo_studio_manager_filter_lostpassword_url', 10, 2 );

/** Keep WordPress-generated registration links inside the theme experience. */
function tempo_studio_manager_filter_register_url( $register_url = '' ) {
	return tempo_studio_manager_login_url( '', 'register' );
}
add_filter( 'register_url', 'tempo_studio_manager_filter_register_url' );

/** Whether the companion plugin can safely create member accounts. */
function tempo_studio_manager_registration_available() {
...
function tempo_studio_manager_registration_values() {
 ...
 */
function tempo_studio_manager_registration_attempt_allowed() {
	$limit  = max( 1, (int) apply_filters( 'tempo_studio_manager_registration_attempt_limit', 5 ) );
	$window = max( MINUTE_IN_SECONDS, (int) apply_filters( 'tempo_studio_manager_registration_attempt_window', 15 * MINUTE_IN_SECONDS ) );
 ...
 */
function tempo_studio_manager_retrieve_password_message( $message, $key, $user_login, $user_data ) {
	...
	$theme_url = add_query_arg(
		...
		tempo_studio_manager_login_url()
	);

	return str_replace( $core_url, $theme_url, $message );
}
add_filter( 'retrieve_password_message', 'tempo_studio_manager_retrieve_password_message', 10, 4 );
...
function tempo_studio_manager_redirect_core_login() {
	...
	$url      = tempo_studio_manager_login_url( $redirect, $action );
...
add_action( 'login_init', 'tempo_studio_manager_redirect_core_login' );

/** Extract and validate a requested post-login destination. */
function tempo_studio_manager_requested_redirect() {
	$value = '';
...
function tempo_studio_manager_reset_key() {
	$cookie_name = 'wp-resetpass-' . COOKIEHASH;
	if ( empty( $_COOKIE[ $cookie_name ] ) || false === strpos( $_COOKIE[ $cookie_name ], ':' ) ) {
		return '';
	}
...
function tempo_studio_manager_clear_reset_cookie() {
	setcookie(
		'wp-resetpass-' . COOKIEHASH,
		' ',
		time() - YEAR_IN_SECONDS,
		(string) wp_parse_url( tempo_studio_manager_login_url(), PHP_URL_PATH ),
		COOKIE_DOMAIN,
		is_ssl(),
		true
	);
...
function tempo_studio_manager_custom_login() {
	if ( ! tempo_studio_manager_is_login_request() ) {
		return;
	}
	...
	$redirect_to     = tempo_studio_manager_requested_redirect();
	$errors          = new WP_Error();
	$user_login      = '';
	$reset_user      = null;
	...
	if ( in_array( $action, array( 'rp', 'resetpass' ), true ) && isset( $_GET['key'], $_GET['login'] ) ) {
		...
		wp_safe_redirect( tempo_studio_manager_login_url( '', 'resetpass' ) );
		exit;
	}
	if ( 'rp' === $action ) {
		$action = 'resetpass';
	...
	if ( 'POST' === strtoupper( isset( $_SERVER['REQUEST_METHOD'] ) ? $_SERVER['REQUEST_METHOD'] : '' ) ) {
		if ( 'login' === $action ) {
			...
			if ( ! isset( $_POST['tempo_login_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['tempo_login_nonce'] ) ), 'tempo_login' ) ) {
				$errors->add( 'invalid_nonce', __( 'Your session expired. Refresh the page and try again.', 'tempo-studio-manager' ) );
			} else {
				$user = wp_signon(
					array(
			...
			$registration = tempo_studio_manager_registration_values();
			...
			if ( ! isset( $_POST['tempo_lostpassword_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['tempo_lostpassword_nonce'] ) ), 'tempo_lostpassword' ) ) {
				...
				if ( ! is_wp_error( $result ) || array_intersect( array( 'invalid_email', 'invalidcombo', 'invalid_username' ), $result->get_error_codes() ) ) {
					wp_safe_redirect( tempo_studio_manager_login_url( $redirect_to, 'checkemail' ) );
					exit;
				}
				$errors = $result;
			}
		} elseif ( 'resetpass' === $action ) {
			$reset_user = tempo_studio_manager_reset_user();
			$cookie_key = tempo_studio_manager_reset_key();
			$post_key   = isset( $_POST['rp_key'] ) ? wp_unslash( $_POST['rp_key'] ) : '';
			if ( is_wp_error( $reset_user ) || ! $cookie_key || ! hash_equals( $cookie_key, $post_key ) ) {
				tempo_studio_manager_clear_reset_cookie();
				wp_safe_redirect( add_query_arg( 'error', 'invalidkey', tempo_studio_manager_login_url( '', 'lostpassword' ) ) );
				exit;
			}
			...
			if ( ! $errors->has_errors() ) {
				...
				tempo_studio_manager_clear_reset_cookie();
				wp_safe_redirect( tempo_studio_manager_login_url( '', 'resetcomplete' ) );
				exit;
			}
		}
	}
	...
	status_header( 200 );
	tempo_studio_manager_render_login( $action, $errors, $redirect_to, $user_login, $reset_user, $registration );
	exit;
}
add_action( 'template_redirect', 'tempo_studio_manager_custom_login', 0 );

/** Return user-facing error copy without exposing which credential failed. */
function tempo_studio_manager_login_error_messages( $errors ) {
	if ( ! $errors->has_errors() ) {
		return array();
	}
 ...
 */
function tempo_studio_manager_render_login( $action, $errors, $redirect_to, $user_login, $reset_user, $registration = array() ) {
	$site_name       = get_bloginfo( 'name' );
	$logo            = tempo_logo_url();
	$panel_image     = tempo_login_panel_image_url();
	$student_word    = function_exists( 'dsb_vocab' ) ? dsb_vocab( 'student' ) : __( 'student', 'tempo-studio-manager' );
	...
	$titles    = array(
		'login'         => __( 'Welcome back', 'tempo-studio-manager' ),
		'register'      => __( 'Create your account', 'tempo-studio-manager' ),
		'registered'    => __( 'Check your email', 'tempo-studio-manager' ),
		'lostpassword'  => __( 'Reset your password', 'tempo-studio-manager' ),
		'checkemail'    => __( 'Check your email', 'tempo-studio-manager' ),
		'resetpass'     => __( 'Choose a new password', 'tempo-studio-manager' ),
		'resetcomplete' => __( 'Password updated', 'tempo-studio-manager' ),
	);
	...
	<body class="tempo-login tempo-login--<?php echo esc_attr( $action ); ?>">
		<a class="tempo-login__skip" href="#tempo-login-form"><?php echo 'register' === $action ? esc_html__( 'Skip to registration', 'tempo-studio-manager' ) : esc_html__( 'Skip to sign in', 'tempo-studio-manager' ); ?></a>
		<main class="tempo-login__shell">
			<section class="tempo-login__form-panel">
				<div class="tempo-login__form-wrap">
					...
					<div class="tempo-login__intro">
						<p class="tempo-login__eyebrow"><?php esc_html_e( 'Member portal', 'tempo-studio-manager' ); ?></p>
					...
					<?php foreach ( tempo_studio_manager_login_error_messages( $errors ) as $message ) : ?>
					...
					<?php if ( in_array( ( isset( $_GET['error'] ) ? sanitize_key( wp_unslash( $_GET['error'] ) ) : '' ), array( 'invalidkey', 'expiredkey' ), true ) ) : ?>
						<div class="tempo-login__notice tempo-login__notice--error" role="alert"><span aria-hidden="true">!</span><p><?php esc_html_e( 'That reset link has expired or was already used. Request a new one below.', 'tempo-studio-manager' ); ?></p></div>
					<?php endif; ?>

					<?php if ( 'login' === $action ) : ?>
						<?php do_action( 'login_form_login' ); ?>
						<form id="tempo-login-form" class="tempo-login__form" action="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to ) ); ?>" method="post">
							<div class="tempo-field">
								<label for="tempo-user-login"><?php esc_html_e( 'Email address or username', 'tempo-studio-manager' ); ?></label>
							...
							<div class="tempo-field">
								<div class="tempo-field__label-row">
									<label for="tempo-password"><?php esc_html_e( 'Password', 'tempo-studio-manager' ); ?></label>
									<a href="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to, 'lostpassword' ) ); ?>"><?php esc_html_e( 'Forgot password?', 'tempo-studio-manager' ); ?></a>
								</div>
								<div class="tempo-field__password">
									<input id="tempo-password" name="pwd" type="password" autocomplete="current-password" required>
									<button class="tempo-password-toggle" type="button" aria-controls="tempo-password" aria-pressed="false"><span class="screen-reader-text"><?php esc_html_e( 'Show password', 'tempo-studio-manager' ); ?></span><span aria-hidden="true"></span></button>
								</div>
							</div>
							<label class="tempo-login__remember"><input name="rememberme" type="checkbox" value="forever"><span><?php esc_html_e( 'Keep me signed in', 'tempo-studio-manager' ); ?></span></label>
							<?php wp_nonce_field( 'tempo_login', 'tempo_login_nonce' ); ?>
							<input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect_to ); ?>">
							<?php do_action( 'login_form' ); ?>
							<button class="tempo-login__submit" type="submit"><?php esc_html_e( 'Sign in', 'tempo-studio-manager' ); ?><span aria-hidden="true">&rarr;</span></button>
						</form>
						<?php if ( tempo_studio_manager_registration_available() ) : ?>
							<p class="tempo-login__register-prompt"><?php esc_html_e( 'New here?', 'tempo-studio-manager' ); ?> <a href="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to, 'register' ) ); ?>"><?php esc_html_e( 'Create an account', 'tempo-studio-manager' ); ?></a></p>
						<?php endif; ?>
					<?php elseif ( 'register' === $action ) : ?>
						<?php do_action( 'login_form_register' ); ?>
						<form id="tempo-login-form" class="tempo-login__form tempo-registration" action="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to, 'register' ) ); ?>" method="post" data-registration-form>
							<fieldset class="tempo-registration__choice">
								<legend><?php esc_html_e( 'Who are you creating this account for?', 'tempo-studio-manager' ); ?></legend>
							...
							<fieldset class="tempo-registration__section">
								<legend><?php echo esc_html( sprintf( __( '%s details', 'tempo-studio-manager' ), $student_label ) ); ?></legend>
								<div class="tempo-registration__row">
									<div class="tempo-field"><label for="tempo-student-first-name"><?php esc_html_e( 'First name', 'tempo-studio-manager' ); ?></label><input id="tempo-student-first-name" name="student_first_name" type="text" value="<?php echo esc_attr( $registration['student_first_name'] ?? '' ); ?>" autocomplete="given-name" maxlength="100" required></div>
									<div class="tempo-field"><label for="tempo-student-last-name"><?php esc_html_e( 'Last name', 'tempo-studio-manager' ); ?></label><input id="tempo-student-last-name" name="student_last_name" type="text" value="<?php echo esc_attr( $registration['student_last_name'] ?? '' ); ?>" autocomplete="family-name" maxlength="100" required></div>
								</div>
							</fieldset>
							...
							<fieldset class="tempo-registration__section tempo-registration__details tempo-registration__details--parent" data-registration-panel="parent">
								<legend><?php esc_html_e( 'Parent or guardian details', 'tempo-studio-manager' ); ?></legend>
								<div class="tempo-registration__row">
									<div class="tempo-field"><label for="tempo-parent-first-name"><?php esc_html_e( 'First name', 'tempo-studio-manager' ); ?></label><input id="tempo-parent-first-name" name="parent_first_name" type="text" value="<?php echo esc_attr( $registration['parent_first_name'] ?? '' ); ?>" autocomplete="given-name" maxlength="100" data-registration-required></div>
									<div class="tempo-field"><label for="tempo-parent-last-name"><?php esc_html_e( 'Last name', 'tempo-studio-manager' ); ?></label><input id="tempo-parent-last-name" name="parent_last_name" type="text" value="<?php echo esc_attr( $registration['parent_last_name'] ?? '' ); ?>" autocomplete="family-name" maxlength="100" data-registration-required></div>
								</div>
								<div class="tempo-field"><label for="tempo-relationship"><?php echo esc_html( sprintf( __( 'Relationship to the %s', 'tempo-studio-manager' ), $student_word ) ); ?></label><input id="tempo-relationship" name="relationship" type="text" value="<?php echo esc_attr( $registration['relationship'] ?? '' ); ?>" placeholder="<?php esc_attr_e( 'For example, parent or guardian', 'tempo-studio-manager' ); ?>" maxlength="80" data-registration-required></div>
								<div class="tempo-registration__row">
							...
							<button class="tempo-login__submit" type="submit"><?php esc_html_e( 'Create account', 'tempo-studio-manager' ); ?><span aria-hidden="true">&rarr;</span></button>
						</form>
						<a class="tempo-login__back" href="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to ) ); ?>">&larr; <?php esc_html_e( 'Back to sign in', 'tempo-studio-manager' ); ?></a>
					<?php elseif ( 'lostpassword' === $action ) : ?>
						<?php do_action( 'login_form_lostpassword' ); ?>
						<form id="tempo-login-form" class="tempo-login__form" action="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to, 'lostpassword' ) ); ?>" method="post">
							<div class="tempo-field">
								<label for="tempo-user-login"><?php esc_html_e( 'Email address or username', 'tempo-studio-manager' ); ?></label>
							...
							<button class="tempo-login__submit" type="submit"><?php esc_html_e( 'Send reset link', 'tempo-studio-manager' ); ?><span aria-hidden="true">&rarr;</span></button>
						</form>
						<a class="tempo-login__back" href="<?php echo esc_url( tempo_studio_manager_login_url( $redirect_to ) ); ?>">&larr; <?php esc_html_e( 'Back to sign in', 'tempo-studio-manager' ); ?></a>
					<?php elseif ( 'resetpass' === $action && $reset_user instanceof WP_User ) : ?>
						<?php do_action( 'login_form_resetpass' ); ?>
						<form id="tempo-login-form" class="tempo-login__form" action="<?php echo esc_url( tempo_studio_manager_login_url( '', 'resetpass' ) ); ?>" method="post">
							<div class="tempo-field">
								<label for="tempo-new-password"><?php esc_html_e( 'New password', 'tempo-studio-manager' ); ?></label>
								<div class="tempo-field__password"><input id="tempo-new-password" name="pass1" type="password" autocomplete="new-password" required autofocus><button class="tempo-password-toggle" type="button" aria-controls="tempo-new-password" aria-pressed="false"><span class="screen-reader-text"><?php esc_html_e( 'Show password', 'tempo-studio-manager' ); ?></span><span aria-hidden="true"></span></button></div>
								<p class="tempo-field__hint"><?php esc_html_e( 'Use at least 12 characters with a mix of words, numbers and symbols.', 'tempo-studio-manager' ); ?></p>
							</div>
							<div class="tempo-field"><label for="tempo-confirm-password"><?php esc_html_e( 'Confirm new password', 'tempo-studio-manager' ); ?></label><div class="tempo-field__password"><input id="tempo-confirm-password" name="pass2" type="password" autocomplete="new-password" required><button class="tempo-password-toggle" type="button" aria-controls="tempo-confirm-password" aria-pressed="false"><span class="screen-reader-text"><?php esc_html_e( 'Show password', 'tempo-studio-manager' ); ?></span><span aria-hidden="true"></span></button></div></div>
							<?php wp_nonce_field( 'tempo_reset_password', 'tempo_reset_nonce' ); ?>
							<input type="hidden" name="rp_key" value="<?php echo esc_attr( tempo_studio_manager_reset_key() ); ?>">
					...
					<p class="tempo-login__privacy">
						<?php esc_html_e( 'Secure access for members of', 'tempo-studio-manager' ); ?> <?php echo esc_html( $site_name ); ?>.
						<?php if ( get_privacy_policy_url() ) : ?><a href="<?php echo esc_url( get_privacy_policy_url() ); ?>"><?php esc_html_e( 'Privacy', 'tempo-studio-manager' ); ?></a><?php endif; ?>
					</p>
				</div>
			</section>
			...
			<aside
				...
				<?php else : ?>
					aria-label="<?php esc_attr_e( 'Your studio portal', 'tempo-studio-manager' ); ?>"
				<?php endif; ?>
			>
				<?php if ( ! $panel_image ) : ?>
				...
				<div class="tempo-login__showcase-content">
					...
					<div class="tempo-portal-card" aria-hidden="true">
						...
						<div class="tempo-portal-card__line"><i></i><div><strong><?php esc_html_e( 'Move', 'tempo-studio-manager' ); ?></strong><small><?php esc_html_e( 'Find your next class', 'tempo-studio-manager' ); ?></small></div><em>&rarr;</em></div>
						<div class="tempo-portal-card__line tempo-portal-card__line--second"><i></i><div><strong><?php esc_html_e( 'Create', 'tempo-studio-manager' ); ?></strong><small><?php esc_html_e( 'Keep every booking together', 'tempo-studio-manager' ); ?></small></div><em>&rarr;</em></div>
					</div>
					<div class="tempo-login__pulse" aria-hidden="true"><span></span><?php esc_html_e( 'Ready when you are', 'tempo-studio-manager' ); ?></div>
				</div>
				<?php endif; ?>
			</aside>
		</main>
